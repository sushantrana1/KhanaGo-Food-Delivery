...
export const createOrder = asyncWrapper(async (req, res) => {
  ...
  const {
    ...
    total,
  } = calculateOrderTotal(validatedItems, 100, codSurcharge, 0);
...
export const getOrders = asyncWrapper(async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: "success", count: orders.length, results: orders });
});
...
export const cancelOrder = asyncWrapper(async (req, res, next) => {
  ...
  if (!["pending", "confirmed"].includes(order.orderStatus)) {
    return next(
      new ApiError("Order cannot be cancelled at this stage", 400)
    );
  }
...
export const trackOrder = asyncWrapper(async (req, res) => {
  ...
  const timeline = statuses.map((status, idx) => ({
    ...
    completed:
      order.orderStatus === "cancelled" ? false : idx <= currentIndex,
    current:
      order.orderStatus === "cancelled" ? false : idx === currentIndex,
  }));
