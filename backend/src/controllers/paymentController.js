...
import {
  initiateKhalti,
  verifyKhalti,
} from "../services/payments/khaltiService.js";
import {
  initiateEsewa,
  verifyEsewa,
} from "../services/payments/esewaService.js";
import {
  initiateImepay,
  verifyImepay,
} from "../services/payments/imepayService.js";
...
export const initiatePayment = asyncWrapper(async (req, res) => {
  ...
  if (provider === "khalti") result = await initiateKhalti(req.user, order);
    ...
    result = { paymentUrl: null, transactionId: `COD-${Date.now()}` };
  ...
  res.status(200).json({
    ...
    formFields: result.formFields || null,
    payment,
  });
});

const verifyPaymentGeneric = async (providerName, verifyFn, req, res) => {
  const { transactionId } = req.body;

  const valid = await verifyFn(transactionId);
...
export const verifyKhaltiPayment = asyncWrapper(async (req, res) =>
  verifyPaymentGeneric("khalti", verifyKhalti, req, res)
);

export const verifyEsewaPayment = asyncWrapper(async (req, res) =>
  verifyPaymentGeneric("esewa", verifyEsewa, req, res)
);

export const verifyImepayPayment = asyncWrapper(async (req, res) =>
  verifyPaymentGeneric("imepay", verifyImepay, req, res)
);