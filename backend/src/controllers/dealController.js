...
export const getDealById = asyncWrapper(async (req, res) => {
  const deal = await Deal.findById(req.params.id).populate(
    "meal",
    "name image category area rating ingredients instructions externalMealId"
  );
  if (!deal || !deal.isActive)
    throw new ApiError("Deal not found or expired", 404);
...
export const claimDeal = asyncWrapper(async (req, res) => {
  ...
  if (deal.endDate < new Date())
    throw new ApiError("Deal has expired", 400);

  if (deal.claimedCount >= deal.maxClaims)
    throw new ApiError("Deal has been fully claimed", 400);
  ...
  res.status(200).json({
    status: "success",
    message: "Deal claimed. Proceed to checkout.",
    data: {
      deal,
      redirectTo: `/buy-now/${deal.meal.externalMealId || deal.meal._id}`,
